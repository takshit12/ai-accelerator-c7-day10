...
# Chain them with LCEL directly -- SimpleSequentialChain (legacy) can't
# accept LCEL Runnables in its chains=[...] list, it expects classic Chain
# objects. This *is* the modern equivalent: pipe, reshaping the dict key
# between stages since idea_chain outputs a bare string but slogan_chain's
# prompt expects {"business_idea": ...}.
# Output from idea_chain automatically becomes input for slogan_chain
simple_sequential_chain = (
    idea_chain
    | RunnableLambda(lambda idea: {"business_idea": idea})
    | slogan_chain
)
...
# Create content pipeline -- same reshape-between-stages pattern as above,
# now three stages deep (topics -> outline -> intro).
content_pipeline = (
    topic_generator
    | RunnableLambda(lambda topics: {"topics": topics})
    | outline_generator
